...
const PALETTE_ITEMS = [
  ...
  { type: 'ADD',           label: 'ADD',     color: '#ec4899', desc: '数値(2〜4入力)を合算する' },
  { type: 'VAR_READ',      label: 'VAR_READ',  color: '#14b8a6', desc: '内部変数を読み取る' },
  { type: 'VAR_WRITE',     label: 'VAR_WRITE', color: '#14b8a6', desc: '内部変数へ書き込む' },
];
...
export function BlockPalette() {
  ...
  const exampleNames = usePLCStore((s) => s.exampleNames);
  const loadExampleNames = usePLCStore((s) => s.loadExampleNames);
  ...
  return (
    ...
    >
      ...
      {exampleNames.length > 0 && (
        <select
          onChange={(e) => e.target.value && loadExampleProgram(e.target.value)}
          defaultValue=""
          title="サンプルプログラムを切り替える"
          style={{
            marginLeft: 'auto',
            background: '#0f172a',
